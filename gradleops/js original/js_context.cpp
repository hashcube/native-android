...
void defMeasureTextBitmap(const v8::FunctionCallbackInfo<v8::Value> &args) {
    ...
    String::Utf8Value text_str(isolate, args[0]);
    const char *text = ToCString(text_str);
    Handle<Object> font_info(args[1]->ToObject(isolate));
...
Handle<ObjectTemplate> get_context_2d_class_template(Isolate *isolate);
...
void context_2d_class_ctor(const v8::FunctionCallbackInfo<v8::Value> &args) {
    LOGFN("ctx2d ctor");
    Isolate *isolate = getIsolate();
    Handle<Object> canvas(args[0]->ToObject(isolate));
    String::Utf8Value str(isolate, args[1]);
    const char *url = ToCString(str);
    int destTex = args[2]->Int32Value(isolate->GetCurrentContext()).ToChecked();
    //Todo verify should Persistent SetWeak be used or simply Handle instead Persistent below on line 709
    Handle<Object> ctx = get_context_2d_class_template(isolate)->NewInstance();
...
Handle<ObjectTemplate> js_gl_get_template(Isolate *isolate) {
    Handle<ObjectTemplate> gl = ObjectTemplate::New(isolate);
