...
public class V8Inspector {
    ...
    public static Pair<String, String>[] getPageResources() {
        ...
        File rootFilesDir = new File(dataDir, "resources");
    ...
    private static List<Pair<String, String>> traverseResources(File dir) {
        ...
        //File f = new File(TeaLeaf.get().getFilesDir().getAbsolutePath(), "/resources/native.js");
        //resources.add(new Pair<>("file://" + f.getAbsolutePath(),  "text/javascript"));
        ...
        //String s = TeaLeaf.get().getLaunchUri();
        //resources.add(new Pair<>(TeaLeaf.get().getLaunchUri(), "text/javascript"));

        return resources;
    }

    public static File copyAssets() {

        CopyAssets.copyJsSourcesAssets(TeaLeaf.get());

        File f = new File(TeaLeaf.get().getFilesDir().getAbsolutePath(), "/resources/native.js");

        if(!f.exists()){
            throw new IllegalArgumentException("No js sources found at path "+f.getAbsolutePath());
        }
    ...
    private class V8InspectorWebSocket extends NanoWSD.WebSocket {
        ...
        protected void onOpen() {
            NativeShim.connect(V8InspectorWebSocket.this);
        }
            ...
            if (!ReadyToProcessMessages.get()) {
               ...
               Runnable r = new Runnable() {
                    ...
                    public void run() {
                        String nextMessage = inspectorMessages.poll();
                        while (nextMessage != null) {
                            NativeShim.dispatchMessage(nextMessage);
                            nextMessage = inspectorMessages.poll();
                        }
                    }
                };
                TeaLeaf.get().runOnUiThread(r);
            }
        }
    ...
    public void start() throws IOException {
        if (this.server == null && BuildConfig.DEBUG) {
 ...
 /*   public void waitForDebugger(boolean shouldBreak, TeaLeafThreadsCallback teaLeafThreadsCallback) {
        if (shouldBreak) {
            synchronized(this.debugLocker) {
                try {
                    ...
                    teaLeafThreadsCallback.runFinally(TeaLeaf.get());
                }
            }
        } else {
            ReadyToProcessMessages.set(true);
    ...
    private void processDebugBreakMessages() {
        while (!pendingInspectorMessages.isEmpty()) {
            ...
            NativeShim.dispatchMessage(inspectorMessage);
        }
    }
